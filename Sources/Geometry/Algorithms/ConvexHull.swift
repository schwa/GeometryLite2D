...
public func convexHullGrahamScan(_ points: [CGPoint], absoluteTolerance: CGFloat = 1e-10) -> [CGPoint] {
    ...
    let sorted = points.filter { $0 != start }.sorted { p1, p2 in
        ...
        if angle1.isApproximatelyEqual(to: angle2, absoluteTolerance: absoluteTolerance) {
