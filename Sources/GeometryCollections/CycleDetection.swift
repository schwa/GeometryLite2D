...
public typealias SimpleGraph<Vertex: Hashable> = [Vertex: Set<Vertex>]

// MARK: - Graph Building

/// Builds an undirected graph from a collection of edges
public func buildGraph<Vertex: Hashable>(from edges: [(Vertex, Vertex)]) -> SimpleGraph<Vertex> {
    var graph: SimpleGraph<Vertex> = [:]
...
public func buildGraph(from segments: [LineSegment]) -> SimpleGraph<CGPoint> {
    var graph: SimpleGraph<CGPoint> = [:]
...
public func findAllCycles<Vertex: Hashable>(in graph: SimpleGraph<Vertex>) -> [[Vertex]] {
...
private func isSimpleLoop<Vertex: Hashable>(_ cycle: [Vertex], in graph: SimpleGraph<Vertex>) -> Bool {
