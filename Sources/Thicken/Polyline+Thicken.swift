...
public extension Polyline {
    /// Returns thickened atoms for this polyline
    /// - Parameters:
    ///   - width: Stroke width
    ...
    func thickened(
        ...
        if closed, vertices.count < 3 {
            return thickened(width: width, joinStyle: joinStyle, capStyle: capStyle, closed: false)
        }

        var segments = zip(vertices, vertices.dropFirst()).map { LineSegment(start: $0, end: $1) }

        // Add closing segment if closed
        if closed, let first = vertices.first, let last = vertices.last, first != last {
